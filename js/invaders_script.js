...
var GameArea = {
    ...
    start: function(){
        ...
        this.context.font = ((window.innerWidth+window.innerHeight)/50)+"px Arial";
        ...
        this.score = 0;
        Hrac = new Player (window.innerWidth*0.1, window.innerHeight/2, "images/player.gif");
        Bos = 0;
        Enemies = [];
        Bullets = [];
        bossBullets = [];
        ...
        window.addEventListener('resize', () =>{
            ...
            Bos.height = (window.innerWidth+window.innerHeight)/50;
            Bos.width = (window.innerWidth+window.innerHeight)/50;
            Hrac.height = (window.innerWidth+window.innerHeight)/50;
            Hrac.width = (window.innerWidth+window.innerHeight)/50;
            for (let i =0 ;i<Enemies.length;i++){
                Enemies[i].height = (window.innerWidth+window.innerHeight)/50;
                Enemies[i].width = (window.innerWidth+window.innerHeight)/50;
            }
            this.clear();
    ...
    updateGameArea: function(){
        ...
        if (GameArea.score%50 == 0 && GameArea.score !=0 && GameArea.bossfight == false){
...
class Player{
    constructor(x, y, imgsrc){
        ...
        this.bulletRate = 0;
        this.explode = false;
        this.height = (window.innerWidth+window.innerHeight)/50;
        this.width = (window.innerWidth+window.innerHeight)/50;
    }
    update(){
    ...
    changePos(){
        ...
        if (this.bulletRate>=20){
            ...
            Bullets.push(new Bullet(this.x + this.width,this.y,10,"images/bullet.png"),new Bullet(this.x + this.width,this.y+this.height,10,"images/bullet.png"));
            this.bulletRate = 0;
            }
        }
        this.x += this.speedX;
    ...
    crash(){
        for (let i=0;i<Enemies.length;i++){
            if ((Enemies[i].x + Enemies[i].width >= this.x && Enemies[i].x <= this.x + this.width) && ((Enemies[i].y + Enemies[i].height >= this.y && Enemies[i].y <= this.y + this.height) ||(this.y + this.height >= Enemies[i].y && this.y <= Enemies[i].y + Enemies[i].height)))
                {this.explode = true;}
        }
    }
    crashBoss(){
        if(this.x <= (Bos.x+Bos.width) && this.x >= Bos.x && this.y>=Bos.y && this.y<=(Bos.height + Bos.y)){
            this.explode = true;
        }
    }
}
...
class Boss{
    ...
    update(){
        this.bulletRate += 1;
        if (this.bulletRate>=20){
            bossBullets.push(new Bullet(this.x,this.y+this.height/2,-10,"images/bullet.png"));
            this.bulletRate = 0;
        }
...
class Bullet{
    ...
    crash(){
        for (let j=0;j<Enemies.length;j++){
            if (this.x <= (Enemies[j].x+Enemies[j].width) && this.x >= Enemies[j].x && this.y>=Enemies[j].y && this.y<=(Enemies[j].height + Enemies[j].y)){
    ...
    crashPlayer(){
        if(this.x <= (Hrac.x+Hrac.image.width) && this.x >= Hrac.x && this.y>=Hrac.y && this.y<=(Hrac.height + Hrac.y)){
            Bullets.splice(Bullets.indexOf(this),1);
            Hrac.explode = true;
        }
    }
